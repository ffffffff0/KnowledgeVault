    ...
    def __call__(self, fnm, binary=None, callback=None):
        ...
        callback(
            ...
            (
                "Extract pairs: {}. ".format(len(res))
                + (
                    f"{len(fails)} failure, line: %s..." % (",".join(fails[:3]))
                    if fails
                    else ""
                )
            ),
...
class Pdf(PdfParser):
    def __call__(
        ...
        tbl_pn, tbl_left, tbl_right, tbl_top, tbl_bottom, tbl_tag, tbl_text = (
            1,
            0,
            0,
            0,
        ...
        for box in self.boxes:
            ...
            has_bull, index = has_qbullet(
                reg, box, last_box, last_index, last_bull, bull_x0_list
            )
            ...
            tbl_pn, tbl_left, tbl_right, tbl_top, tbl_bottom, tbl_tag, tbl_text = (
                self.get_tbls_info(tbls, tbl_index)
            )
            if not has_bull:  # No question bullet
                if not last_q:
                    if tbl_pn < line_pn or (
                        tbl_pn == line_pn and tbl_top <= line_top
                    ):  # image passed
                        tbl_index += 1
                    continue
                else:
                    ...
                    sum_section = section
                    while (
                        (tbl_pn == last_pn and tbl_top >= last_bottom)
                        or (tbl_pn > last_pn)
                    ) and (
                        (tbl_pn == line_pn and tbl_top <= line_top)
                        or (tbl_pn < line_pn)
    ...
    def get_tbls_info(self, tbls, tbl_index):
        ...
        tbl_tag = "@@{}\t{:.1f}\t{:.1f}\t{:.1f}\t{:.1f}##".format(
            tbl_pn, tbl_left, tbl_right, tbl_top, tbl_bottom
        )
...
def chunk(
    ...
    **kwargs,
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
            ...
            if sum_question:
                res.append(
                    beAdoc(
                        ...
                        sum_question,
                        markdown(
                            last_answer, extensions=["markdown.extensions.tables"]
                        ),
                        eng,
                        index,
                    )
        ...
        docx_parser = Docx()
        qai_list, tbls = docx_parser(
            filename, binary, from_page=0, to_page=10000, callback=callback
        )
