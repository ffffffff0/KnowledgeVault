...
def chunk(
    ...
    **kwargs,
    ...
    doc["title_tks"] = rag_tokenizer.tokenize(
        re.sub(r"\.[a-zA-Z]+$", "", doc["docnm_kwd"])
    )
    ...
    if re.search(r"\.pdf$", filename, re.IGNORECASE):
        ...
        if len(sections) > 0 and len(pdf_parser.outlines) / len(sections) > 0.03:
            ...
            for txt, _, _ in sections:
                for t, lvl in pdf_parser.outlines:
                    ...
                    tks_ = set(
                        [txt[i] + txt[i + 1] for i in range(min(len(t), len(txt) - 1))]
                    )
        ...
        sections = [
            (txt, sec_ids[i], poss) for i, (txt, _, poss) in enumerate(sections)
        ]
