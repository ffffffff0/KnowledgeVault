...
from deepdoc.parser.figure_parser import (
    VisionFigureParser,
    vision_figure_parser_figure_data_wrapper,
)
...
class Docx(DocxParser):
    ...
    def __get_nearest_title(self, table_index, filename):
        ...
        try:
            ...
            for i, block in enumerate(self.doc._element.body):
                if block.tag.endswith("p"):  # Paragraph
                    ...
                    blocks.append(
                        ("t", i, None)
                    )  # Table object will be retrieved later
        ...
        for i in range(len(blocks) - 1, -1, -1):
            ...
            if (
                block.style
                and block.style.name
                and re.search(r"Heading\s*(\d+)", block.style.name, re.I)
            ):
                try:
        ...
        if nearest_title:
            ...
            while current_level > 1:
                ...
                for i in range(len(blocks) - 1, -1, -1):
                    ...
                    if block.style and re.search(
                        r"Heading\s*(\d+)", block.style.name, re.I
                    ):
                        try:
                            level_match = re.search(r"(\d+)", block.style.name)
                            if level_match:
...
class Pdf(PdfParser):
    ...
    def __call__(
        ...
        if separate_tables_figures:
            ...
            return (
                [(b["text"], self._line_tag(b, zoomin)) for b in self.boxes],
                tbls,
                figures,
            )
        else:
    ...
    **kwargs,
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        try:
            ...
            callback(
                0.15,
                "Visual model detected. Attempting to enhance figure extraction...",
            )
        except Exception:
            vision_model = None
        ...
        if vision_model:
            ...
            try:
                docx_vision_parser = VisionFigureParser(
                    vision_model=vision_model, figures_data=figures_data, **kwargs
                )
                ...
                callback(
                    0.6,
                    f"Visual model error: {e}. Skipping figure parsing enhancement.",
                )
        ...
        if layout_recognizer == "DeepDOC":
            ...
            if vision_model:
                ...
                callback(
                    0.5, "Basic parsing complete. Proceeding with figure enhancement..."
                )
                try:
                    pdf_vision_parser = VisionFigureParser(
                        vision_model=vision_model, figures_data=figures, **kwargs
                    )
        ...
        for section_text, _ in sections:
            images = (
                markdown_parser.get_pictures(section_text) if section_text else None
            )
            if images:
                # If multiple images found, combine them using concat_img
                combined_image = (
                    reduce(concat_img, images) if len(images) > 1 else images[0]
                )
