...
async def do_handle_task(task):
    ...
    if task.get("task_type", "") == "raptor":
        ...
        chat_model = LLMBundle(
            task_user_id, LLMType.CHAT, llm_name=task_llm_id, lang=task_language
        )
        # run RAPTOR
        ...
        try:
            ...
            print(f"Embedding done for task {task_id}, token count: {token_count}, vector size: {vector_size}")
        except Exception as e:
    ...
    for b in range(0, len(chunks), DOC_BULK_SIZE):
        ...
        print(f"result of insert chunks {b} to {b + DOC_BULK_SIZE} for task {task_id}: {doc_store_result}")
        task_canceled = TaskService.do_cancel(task_id)
        if task_canceled:
            progress_callback(-1, msg="Task has been canceled.")
            return
        ...
        try:
            ...
            doc_store_result = await trio.to_thread.run_sync(
                lambda: mc.delete(
                    f"kb_id == {task_dataset_id} AND id IN {[f'{chunk_id}' for chunk_id in chunk_ids]}",
                )
            )
            async with trio.open_nursery() as nursery:
