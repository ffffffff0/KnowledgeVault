...
def set_api_key(req: SetLLMRequest):
    ...
    for llm in LLMService.query(fid=factory):
        if not embd_passed and llm.model_type == LLMType.EMBEDDING.value:
            assert (
                factory in EmbeddingModel
            ), f"Embedding model from {factory} is not supported yet."
            mdl = EmbeddingModel[factory](req.api_key, llm.llm_name)
            try:
            ...
            assert (
                factory in ChatModel
            ), f"Chat model from {factory} is not supported yet."
            mdl = ChatModel[factory](req.api_key, llm.llm_name)
            try:
                m, tc = mdl.chat(
                    None,
            ...
            assert (
                factory in RerankModel
            ), f"Re-rank model from {factory} is not supported yet."
            mdl = RerankModel[factory](req.api_key, llm.llm_name)
            try:
    ...
    llm_config = {"api_key": req.api_key, "api_base": ""}
