...
class FileService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def get_root_folder(cls, user_id):
        ...
        for file in cls.model.select().where(
            (cls.model.user_id == user_id), (cls.model.parent_id == cls.model.id)
        ):
            return file.to_dict()
    ...
    def new_a_file_from_kb(
        cls, user_id, name, parent_id, ty=FileType.FOLDER.value, size=0, location=""
    ):
        ...
        for file in cls.query(user_id=user_id, parent_id=parent_id, name=name):
            return file.to_dict()
        file = {
            "id": get_uuid(),
            "parent_id": parent_id,
    ...
    @classmethod
    @DB.connection_context()
    def init_knowledgebase_docs(cls, root_id, user_id):
        ...
        for _ in cls.model.select().where(
            ...
            return
        folder = cls.new_a_file_from_kb(user_id, KNOWLEDGEBASE_FOLDER_NAME, root_id)

        for kb in Knowledgebase.select(*[Knowledgebase.id, Knowledgebase.name]).where(
            Knowledgebase.user_id == user_id
        ):
            kb_folder = cls.new_a_file_from_kb(user_id, kb.name, folder["id"])
            for doc in DocumentService.query(kb_id=kb.id):
                FileService.add_file_from_kb(doc.to_dict(), kb_folder["id"], user_id)
    ...
    async def upload_document(self, kb, file_objs, user_id):
        ...
        kb_folder = self.new_a_file_from_kb(user_id, kb.name, kb_root_folder["id"])
        ...
        for file in file_objs:
            try:
                ...
                if (
                    ...
                    and DocumentService.get_doc_count(kb.user_id)
                ...
                FileService.add_file_from_kb(doc, kb_folder["id"], kb.user_id)
