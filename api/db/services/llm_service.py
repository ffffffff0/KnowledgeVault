...
class LLMBundle:
    def __init__(self, user_id, llm_type, llm_name=None, lang="Chinese"):
        ...
        self.mdl = TenantLLMService.model_instance(user_id, llm_type, llm_name, lang=lang)
        assert self.mdl, "Can't find model for {}/{}/{}".format(user_id, llm_type, llm_name)
        model_config = TenantLLMService.get_model_config(user_id, llm_type, llm_name)
    ...
    def encode(self, texts: list):
        ...
        if not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens, llm_name):
            logging.error("LLMBundle.encode can't update token usage for {}/EMBEDDING used_tokens: {}".format(self.user_id, used_tokens))
    ...
    def encode_queries(self, query: str):
        ...
        llm_name = getattr(self, "llm_name", None)
        if not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens, llm_name):
            logging.error("LLMBundle.encode_queries can't update token usage for {}/EMBEDDING used_tokens: {}".format(self.user_id, used_tokens))
    ...
    def similarity(self, query: str, texts: list):
        ...
        if not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens):
            logging.error("LLMBundle.similarity can't update token usage for {}/RERANK used_tokens: {}".format(self.user_id, used_tokens))
    ...
    def describe(self, image, max_tokens=300):
        ...
        if not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens):
            logging.error("LLMBundle.describe can't update token usage for {}/IMAGE2TEXT used_tokens: {}".format(self.user_id, used_tokens))
    ...
    def transcription(self, audio):
        ...
        if not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens):
            logging.error("LLMBundle.transcription can't update token usage for {}/SEQUENCE2TXT used_tokens: {}".format(self.user_id, used_tokens))
    ...
    def tts(self, text):
        ...
        for chunk in self.mdl.tts(text):
            if isinstance(chunk, int):
                if not TenantLLMService.increase_usage(self.user_id, self.llm_type, chunk, self.llm_name):
                    logging.error("LLMBundle.tts can't update token usage for {}/TTS".format(self.user_id))
                return
            yield chunk
    ...
    def chat(self, system, history, gen_conf):
        ...
        if isinstance(txt, int) and not TenantLLMService.increase_usage(self.user_id, self.llm_type, used_tokens, self.llm_name):
            logging.error("LLMBundle.chat can't update token usage for {}/CHAT llm_name: {}, used_tokens: {}".format(self.user_id, self.llm_name, used_tokens))
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        for txt in chat_streamly(system, history, gen_conf):
            ...
            yield ans
        if total_tokens > 0:
            if not TenantLLMService.increase_usage(self.user_id, self.llm_type, txt, self.llm_name):
                logging.error("LLMBundle.chat_streamly can't update token usage for {}/CHAT llm_name: {}, content: {}".format(self.user_id, self.llm_name, txt))
